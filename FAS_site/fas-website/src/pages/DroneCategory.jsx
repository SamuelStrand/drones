...
function PlaceholderImage({ label }) {
  return (
    <div className="relative h-44 w-full overflow-hidden rounded-xl bg-gradient-to-br from-white/10 via-white/5 to-transparent">
      ...
      <div className="h-full w-full flex items-center justify-center px-4">
        <span className="text-white/20 font-black tracking-widest uppercase text-sm text-center">
          {label}
        </span>
      </div>
    </div>
...
export default function DroneCategory() {
  ...
  if (!category) {
    return (
      <PageTransition>
        <SEO title={t("drones_category.not_found.title")} description={t("drones_category.not_found.desc")} path={`/drones/${categorySlug}`} noindex />
  ...
  return (
    <PageTransition>
      <SEO title={t("seo.drones_category.title")} description={t("seo.drones_category.description")} path={`/drones/${categorySlug}`} />

    ...
    <section className="bg-black py-24 px-10">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl md:text-5xl font-bold text-white mb-12 tracking-tight">
          {t('products.title')} <span className="text-blue-800">{t('products.subtitle')}</span>
        </h2>
          ...
          {products.map((p) => (
            <div key={p.id} className="group cursor-pointer">
              <div className="relative aspect-square overflow-hidden bg-zinc-900 rounded-sm">
                <img src={p.image} alt={t(`products.${p.key}.name`)} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100" />
              </div>
              <div className="mt-6">
                <h3 className="text-white text-xl font-bold">{t(`products.${p.key}.name`)}</h3>
                <p className="text-zinc-400 text-sm mt-1">{t(`products.${p.key}.tagline`)}</p>
                <Link to="/drones" className="inline-block mt-4 text-blue-800 text-xs font-bold uppercase tracking-widest border-b border-transparent hover:border-blue-800 transition-all">
